...
ES_PATH_CONF="{{elasticsearch_home}}/config"

# Maximum number of VMA (Virtual Memory Areas) a process can own
MAX_MAP_COUNT=262144

# Elasticsearch PID file directory
PID_DIR="{{elasticsearch_home}}/bin"
...
case "$1" in
  start)
	...
	# Start Daemon
	start-stop-daemon -d $ES_HOME --start --user {{user_name}} -c {{user_name}} --pidfile "$PID_FILE" --exec $DAEMON -- $DAEMON_OPTS
	return=$?
	if [ $return -eq 0 ]; then
		i=0
		timeout=10
