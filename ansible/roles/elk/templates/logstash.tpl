...
# Directory where the Logstash binary distribution resides
LOGSTASH_HOME={{logstash_home}}

# Logstash PID file directory
PID_DIR="{{logstash_home}}/bin"
...
if [ ! -x "$DAEMON" ]; then
	echo "The Logstash startup script does not exists or it is not executable, tried: $DAEMON"
	exit 1
fi
...
case "$1" in
  start)
	...
	pid=`pidofproc -p $PID_FILE logstash`
	if [ -n "$pid" ] ; then
  ...
  stop)
	...
	;;
  status)
	status_of_proc -p $PID_FILE logstash logstash && exit 0 || exit $?
	;;
